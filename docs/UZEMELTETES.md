...
- **56 aktív forrás** (A-kaszt; 2026-08-28: a végleg megszűnt **szabadeu nyugdíjazva** — a
  ...
  **+ 6. csomag SCRAPE-only:** `polcapital` (Political Capital) + `policysol` (Policy Solutions), list_url + per-forrás htmllist-parser. **+ OECD** (`oecd`, api.oecd.org RSS + HU title_filter; a www.oecd.org 403-blokk, az API a működő csatorna — runner-elérhetőség a futáson igazolandó).
