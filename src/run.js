...
async function main() {
  ...
  const run = {
    ...
    notCovered: [
      ...
      "Dedup(b) láncolódó over-merge, rendszerszintű mega-blob — a mai adaton a legnagyobb story-csoport 455 tag (Paks + politika + szereplőnév-láncok). Ok: nagy gyakoriságú közös tokenek (szereplőnevek: 'orbán viktor', 'magyar péter') hamis éleket húznak KÜLÖNBÖZŐ sztorik közt, a union-find tranzitív closure-je egy blobbá láncolja. A tranzitivitás-korlát (C-star: tag csak KÖZVETLEN éllel a reprezentánshoz) 455→17-re zsugorít, DE 185 VALÓDI (dice≥0.55 parafrázis) same-story merge-et is szétbont, mert a rephez csak láncon kötő, egymással near-duplicate tagokat árvázza. A finomított fix: a leszakadt tagokat rekurzívan újracsoportosítani (nem árvaként hagyni) — algoritmus-változás + same-story-megőrző teszt, külön tétel (F3, dedup(b)/C-star). Az Eurostat-6-blob ettől FÜGGETLEN, kisebb eset: a generikus-token (euro/area) fix oldja, 9 pár 0 kárral (shippelve 2026-08-06, title_generic_tokens).",
